...
10. 批量 JSX 限制为最多 100 个文件、总计 100 MiB，单文件仍为 10 MiB；服务端重新计算 SHA256、按已存在 Slug 匹配并拒绝重复、歧义和相同历史内容。
11. 批量发布会锁定批次、实验和版本，复核全部构建成功且每个实验仍指向建批时的版本后，再在单个数据库事务中整体切换。
12. 版本批量清理只接受非当前、从未发布、非排队/构建中的版本，并在删除事务内再次锁定复核；发布过的回滚历史始终受保护。

## 密钥与日志

生产密钥只放 VPS `.env`，镜像、仓库和 Release 不包含令牌或上传资产。管理员密码使用 Argon2id；Session 只在数据库保存带服务器密钥的 HMAC，日志会脱敏 Cookie、Authorization 和 password。审计日志覆盖认证、创建、上传、知识点更新、单独/批量发布、回滚、下架、归档与单独/批量删除。
