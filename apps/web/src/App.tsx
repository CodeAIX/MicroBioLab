...
type Experiment = {
  ...
  active_version_number?: number; active_source_sha256?: string; active_version_published_at?: string;
  version_count?: number; updated_at: string; knowledge_review?: string;
...
function BatchUpdateModal({ batchId, onClose, onChanged }: { batchId?: string; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  return <div className="modal management-modal" role="presentation"><section className="management-modal-card" role="dialog" aria-modal="true" aria-label="批量更新 JSX"><header><div><p className="eyebrow">BATCH UPDATE</p><strong>批量更新 JSX</strong></div><button type="button" onClick={onClose} aria-label="关闭">×</button></header><div className="management-modal-body"><ErrorNotice error={error} />
    {!batch && <section className="batch-picker"><label className="batch-directory-picker">选择包含多个实验包的目录<input type="file" accept=".jsx,text/jsx" multiple {...({ webkitdirectory: "", directory: "" } as Record<string, string>)} onChange={(event) => void chooseDirectory(event)} disabled={busy} /><span>递归读取子目录；支持 &lt;slug&gt;-vsim.jsx 或 &lt;slug&gt;.jsx，最多 {BATCH_JSX_MAX_FILES} 个文件。</span></label>{busy && !preflight && <Loading />}</section>}
...
function ExperimentDetail() {
  ...
  return <div className="experiment-detail-page"><PageHeader eyebrow="EXPERIMENT DETAIL" title={item.title}><div className="header-actions">{item.status === "published" && <><Link className="button" to={publicPath} target="_blank">打开介绍页</Link><Link className="button" to={`${publicPath}/run`} target="_blank">打开正式实验</Link></>}<Link className="button ghost" to="/admin/experiments">← 返回列表</Link></div></PageHeader><ErrorNotice error={error} />
    ...
    <section className="panel version-history-panel"><div className="panel-heading"><div><h2>版本历史</h2><p>每个版本发布后保持不可变，可随时切换生效版本。</p></div><form className="inline-upload" onSubmit={upload}><input name="jsx" type="file" accept=".jsx" required /><button className="button primary">上传新版</button></form></div>{versions.length === 0 ? <Empty>暂无版本</Empty> : <div className="version-list">{versions.map((version) => {
      const canDelete = !version.published_at && item.active_version_id !== version.id && !["queued", "building", "running"].includes(version.status) && !["queued", "running"].includes(version.job_status);
      return <article key={version.id} className={item.active_version_id === version.id ? "active-version" : ""}>
        <div className="version-card-content">
          <div className="version-head"><div><strong>{versionName(version.version_number)}</strong>{item.active_version_id === version.id && <span className="current">当前</span>}{version.published_at && <span className="published-once">已发布记录</span>}</div></div>
          <dl><div><dt>Source SHA256</dt><dd title={version.source_sha256}>{version.source_sha256}</dd></div><div><dt>Builder</dt><dd>{version.builder_version ?? "等待构建"}</dd></div><div><dt>Imports</dt><dd>{version.build_imports?.join(", ") || "—"}</dd></div><div><dt>精确时间</dt><dd className="version-times"><span>上传 {formatDateTime(version.created_at)}</span><span>构建 {formatDateTime(version.built_at)}</span><span>发布 {formatDateTime(version.published_at)}</span></dd></div></dl>
          {version.build_warnings?.map((warning) => <p className="warning" key={warning}>⚠ {warning}</p>)}
          {version.error_message && <div className="notice error"><strong>{version.error_code}</strong><span>{version.error_message}</span></div>}
        </div>
        <aside className="version-card-side"><Status value={version.status} /><div className="version-actions">{canDelete && <button className="text-danger" onClick={() => void removeVersion(version)}>删除未发布版本</button>}{version.status === "success" && <><button onClick={() => setPreview(version)}>预览</button><button className="primary-small" onClick={() => void action(`/api/versions/${version.id}/publish`)}>{item.active_version_id ? "发布 / 回滚至此版本" : "发布"}</button></>}</div></aside>
      </article>;
    })}</div>}</section>
    ...
    {preview && <div className="modal"><div className="modal-card"><header><strong>预览 {versionName(preview.version_number)}</strong><button onClick={() => setPreview(undefined)}>×</button></header><iframe title="管理员预览" src={`/preview/${preview.id}/index.html`} sandbox="allow-scripts allow-forms" referrerPolicy="no-referrer" /></div></div>}</div>;
}
